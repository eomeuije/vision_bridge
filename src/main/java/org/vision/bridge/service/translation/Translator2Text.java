...
public class Translator2Text {
    ...
    public static String translate(String braille) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            for (int j = 0; j < word.length(); j++) {
                ...
                if (existMap == choMap) {
                    int jungEndIndex = end + 1 + getJungEndIndex(word.substring(j + end + 1));
                    ...
                    if (JamoUtils.isOnlyJamo(korean)) {
                        if (jongEndIndex - end >= 1) { // 약자 + 종성인 경우
                            String abb = abbAMap.get(word.substring(j, j + end + 1));
                            List<String> strings = JamoUtils.splitOne(abb);
                            String s = word.substring(j + jongEndIndex, j + jongEndIndex + 1);
                            korean = JamoUtils.combine(strings.get(0), strings.get(1), jongMap.get(s));
                            j += jongEndIndex;
                            ...
                            if (abbEndIndex > 0) { // abbB 약자
                                ...
                                List<String> strings = JamoUtils.splitOne(abb);
                                korean = JamoUtils.combine(choMap.get(word.substring(j, j + end + 1)), strings.get(1), strings.get(2));
                                j += abbEndIndex;
                            } else { // abbA 약자
                                korean = abbAMap.get(word.substring(j, j + end + 1));
                                j += jongEndIndex;
                            }
                        }
                    } else {
                    ...
                    String jung = jungMap.get(word.substring(j, j + end + 1));
                    int jongEndIndex = end + 1 + getJongEndIndex(word.substring(j + end + 1));
                    String korean = JamoUtils.combine("ㅇ", jung, jongMap.get(word.substring(j + end + 1, j + jongEndIndex + 1)));
                    text.append(korean);
                    j += jongEndIndex;
                    ...
                    String korean = JamoUtils.combine(splitOne.get(0), splitOne.get(1), jongMap.get(word.substring(j + end + 1, j + jongEndIndex + 1)));
                    text.append(korean);
                    j += jongEndIndex;
                } else if (existMap == abbBMap) {
                    String abb = abbBMap.get(word.substring(j, j + end + 1));
                    text.append(abb);
                    j += end;
                }
            }
    ...
    private static String translateKoreanByIndex(String target, int choIndex, int jungIndex, int jongIndex) {
        ...
        return JamoUtils.combine(choMap.get(cho), jungMap.get(jung), jongMap.get(jong));
    }
